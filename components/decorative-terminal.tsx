...
export default function DecorativeTerminal({ terminal }: { terminal: Terminal }) {
  return (
    <aside aria-hidden="true" className="terminal-window w-full max-w-[32rem] overflow-hidden rounded-xl border border-border bg-card/95 shadow-2xl shadow-black/20">
      <div className="flex items-center gap-2 border-b border-border bg-secondary/70 px-4 py-3">
        ...
        <span className="min-w-0 truncate pl-2 font-mono text-[10px] text-muted-foreground">{terminal.windowTitle}</span>
      </div>
      <div className="space-y-4 p-4 font-mono text-[0.6875rem] leading-relaxed sm:p-5 sm:text-xs">
        {terminal.entries.map((entry) => (
          <div key={entry.command}>
            ...
            <div className="break-words pl-4 text-muted-foreground">
